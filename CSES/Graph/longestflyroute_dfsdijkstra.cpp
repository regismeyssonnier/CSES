...
vector<bool> vis(100005, false), vistn(100005, false);
deque<int>path;
...
void FindMxPath(int start, int n, vector<AStarNode>& nodessh, vector<vector<pair<ll, ll>>>& g, vector<int>&path) {
    ...
    if (nodessh[n].cost != numeric_limits<ll>::max() / 2) {
        ...
        while (currentNode != nullptr && currentNode->dir != 'A') {
            path.push_back(currentNode->node.second);
...
int maximum = 0;
vector<int> longesttrip(100005, 0);
void findLongestTrip(int node) {
    
   
    vistn[node] = true;
      
    rep(i, g2[node].size()) {
        if (!vistn[g2[node][i].second]) {
            findLongestTrip(g2[node][i].second);
        }

        if ((dp[g2[node][i].second] != -1) && (dp[g2[node][i].second] + 1 > dp[node])) {
            dp[node] = dp[g2[node][i].second] + 1;
            longesttrip[node] = g2[node][i].second;
        }
...
void solve() {
    ...
    vector<int> path;
    //FindMxPath(1, n, nodessh, g2, path);
    dp[n] = 1;
    findLongestTrip(1);
