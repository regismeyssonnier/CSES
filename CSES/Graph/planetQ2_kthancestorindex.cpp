...
void solve() {
    ...
    rep(i, q) {
        ...
        input x >> y;
        int xx = kth_ancestor(x, len[x]); //end of the cycle (Case: x and y are in cycle such that x appears after y)
        if (kth_ancestor(x, len[x] - len[y]) == y) //if y is infront of x
            cout << len[x] - len[y] << endl;
        else if (kth_ancestor(xx, len[xx] - len[y]) == y)
