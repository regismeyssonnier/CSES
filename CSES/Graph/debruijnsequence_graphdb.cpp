...
struct Edge {
    ...
    Edge(int _adj, int _v) : ng(_adj), val(_v) {}
};

string path;
string path2;
vector<list<Edge>> lg;
void Hierholzer(int v, string vl) {

    
    while (lg[v].size() > 0) {
        
        int adj = lg[v].front().ng;
        string vl = to_string(lg[v].front().val);
        //aff "--" espaff  v espaff lg[v].front().ng espaff vl endf
...
void solve() {
    ...
    for (int i = 0; i < pow(2, n)/2;++i) {
        ...
        //aff i espaff((i << 1) & (p)) endf
        //aff i espaff(((i << 1)+1) & (p)) endf

        lg[i].emplace_back((i<<1)&(p), 0);
        lg[i].emplace_back(((i << 1)+1) & (p), 1);
