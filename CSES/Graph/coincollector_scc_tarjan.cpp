...
void solve() {
    ...
    for (int i = 0; i < scc.size(); ++i) {
        aff "scc " espaff i + 1 espaff ":" espaffe
        for (int j = 0; j < scc[i].size(); ++j) {
            aff scc[i][j] espaffe
        }aff endlf
    }

    vector<vector<int>> schcomps(n + 5);
    vector<ll> dp(scc.size() + 5, -1);
    vector<ll>val(scc.size()+5);
    repae(i, 1, n) {
        repj(j, g[i].size()) {
            if (comp[i] != comp[g[i][j]]) {
                schcomps[comp[i]].push_back(comp[g[i][j]]);
            }
        }
        val[comp[i]] += rcoins[i - 1];
    }

    for (int i = 0; i <= n; ++i) {
        aff "sch " espaff i + 1 espaff ":" espaffe
            for (int j = 0; j < schcomps[i].size(); ++j) {
                aff schcomps[i][j] espaffe
            }aff endlf
    }
    ...
    while (!topnode.empty()) {
        curr = topnode.top();
        topnode.pop();
        for (int j = 0; j < schcomps[curr].size(); ++j) {
            aff "enter" endf
            dp[schcomps[curr][j]] = max(dp[schcomps[curr][j]], dp[curr] + val[schcomps[curr][j]]);
        }
        
    }

    aff *max_element(dp.begin(), dp.end()) endf
