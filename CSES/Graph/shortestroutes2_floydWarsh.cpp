...
void solve() {
    ...
    rep(i, m) {
        ...
        adj[a - 1][b - 1] = min(adj[b - 1][a - 1], min(adj[a - 1][b - 1], c));
        adj[b - 1][a - 1] = min(adj[b - 1][a - 1], min(adj[a - 1][b - 1], c));
    ...
    rep(i, q) {
        ...
        if (distances[a-1][b-1]>= numeric_limits<ll>::max() / 2) {
            aff - 1 endf;
        }
        else {
            
            aff distances[a - 1][b - 1] endf;
