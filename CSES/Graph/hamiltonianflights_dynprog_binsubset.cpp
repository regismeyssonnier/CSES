...
void solve() {
    ...
    bool ans = false;
    vector<vector<int>>dp(n + 1, vector<int>((1 << 20) + 1, 0));
    vector<vector<ll>> gadj(n+1, vector<ll>(n+1, 0));
    ...
    for (int i = 0; i < (1 << n); ++i) {
        ...
        for (int k = 0; k < n; ++k) {
            ...
            for (int j = 0; j < n; ++j) {
                ...
                if (gadj[j][k] && (i & (1 << j))) {
                    //i ^ (1 << k) == precedent de destination k
                    ll r = (dp[j][i ^ (1 << k)] * gadj[j][k]) % mod;
                    dp[k][i] = (dp[k][i] + r)%mod;
                    //dp[k][i] %= mod;
                }
