...
int n;
vector<vector<ll>> capacity(500+1, vector<ll>(500+1, 0));
vector<vector<ll>> adje(500 + 1, vector<ll>(500 + 1, 0));
...
int bfs(int s, int t, vector<int>& parent, int n) {
    ...
    while (!q.empty()) {
        ...
        for (int i = 1; i <= n;++i){// int next : adje[cur]) {
            if (adje[cur][i] > 0) {
                int next = i;
                //aff next endf
                if (parent[next] == -1 && capacity[cur][next]) {
                    parent[next] = cur;
                    ll new_flow = min(flow, capacity[cur][next] * adje[cur][next]);
                    if (next == t)
                        return new_flow;
...
void solve() {
    ...
    aff EdmondKarp(gd2, gd3, 1, n, n) endf;
    //aff maxflow(1, n, n) endf;
}
