...
void dfs1(int u) {
    aff "node " espaff u endf
    for (int v : g[u]) {
        if (!vis[v]) {
            aff "node " espaff v espaff "+1" espaff u endf
            ans[v] = ans[u] + 1;
            rep(i, 6)aff "-" espaff ans[i] espaffe
                aff endlf;
            vis[v] = true;
            dfs1(v);
        }
...
void dfs2(int u, int d = 1) {
    ...
    int v = t[u];
    if (vis[v]) {
        aff "vis " espaff v endf
        ans[u] = d;
        rep(i, 6)aff "+" espaff ans[i] espaffe
            aff endlf;
    }
    else {
        aff "not vis suiv" espaff  u espaff "=" espaff v endf
        dfs2(v, d + 1);
...
void solve() {
    ...
    for (int i = 1; i <= n; i++)
        if (in[i] == 0) {
            q.push(i);
            aff "push " espaff i endf
        }

    while (!q.empty()) {
        int u = q.front(); q.pop();
        int v = t[u];
        aff "dec " espaff v endf
        in[v]--;
        if (in[v] == 0)
            q.push(v);
    }
    ...
    for (int i = 1; i <= n; i++)
        if (in[i] && !vis[i]) {
            aff "in " espaff i espaff in[i] endf
            dfs2(i);
